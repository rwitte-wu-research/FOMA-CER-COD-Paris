...
**A.11 Convergence protocol for the 3L-RVE spine (closes F62; 2026-07-16).** Parameterization first: factor-only models (incl. period × moderator panels) run in cell-means by default [T2 convention; DEC-031e]. Models with continuous regressors use a deterministic optimizer ladder `nlminb → optim/BFGS → optim/Nelder-Mead` — first metafor-certified fit wins, optimizer disclosed per fit in run_meta, fallback-produced rows flagged. Identity-check tolerances: cross-optimizer / cross-parameterization 1e-5 absolute on coefficients (probe-measured cross-optimizer delta ≈ 2.5e-6 at the sigma2_study boundary), same-optimizer anchors 1e-6. All rungs failing = stop + parameterization review. Recommended per-run hardening: one BFGS control re-fit, |Δβ| < 1e-5. Evidence and rules: DEC-031f.

**A.12 Block-D execution pins (supersedes the §8.2 snippet and stale §8 quantities; 2026-07-18, ships with the T5 package commit).** Selection models run on one-effect-per-**cluster** aggregates (metafor `aggregate.escalc`, cluster = `cluster_id`, struct = "CS", ρ = 0.6 — the T1/A5 convention; k re-derives on v12 = 114 aggregates), not "one-effect-per-study (k = 66)". The 3PSM base model is `rma(yi, vi, method = "ML")` — the §8.2 `method = "FE"` snippet is superseded (an FE base yields a 2-parameter selection model, not the pinned 3PSM {μ, τ², δ}) [DEC-031g/F63]. Direction pins: `selmodel(alternative = "less")` ≡ `puni_star(side = "left")` — selection favors significant negative effects. The PET→PEESE rule is an alignment rule: both models are always computed and reported (DEC-010); the rule selects the claim-carrying estimate via the CR2/Satterthwaite PET intercept at two-sided α = .05, identically in every subgroup. sez ≡ sqrt(vi), derived in-script (the raw SE column is never read). "RoBMA deferred (E4)" is superseded: RoBMA = N3 with prior sets per DEC-031b, executed in the H-session; the Block-D D3 row cross-references it. §8's legacy subgroup PET values are draft-era placeholders; all Block-D quantities re-derive on v12. Governing entry: DEC-031g.
**A.13 E/H-battery execution pins (supersedes the affected A.6 quantities; 2026-07-22, ships with the DEC-045 package commit).** Block E and Block H execute in three runs — **TH-a** (E1–E3, H4 point estimates + candidate set, H6–H11; ≈ 45–70 min) · **TH-b** (H2a/H2b RoBMA, after the F66 lockfile closure and R↔JAGS handshake) · **TH-c** (H1, H3, H5 + H4 permutation distribution; one overnight run ≈ 13–15 h @ W = 11) — with deterministic row budgets and paired verifiers. Superseded A.6/Annex-H quantities (authority: DEC-045 supersession register S1–S9): N5 codings 6 → 10 (the T2/B3 set; 1,260 mechanically derived cells; joint-inference core = the 10-cell B3 coding family, joint cluster-level permutation, B = 500) [S2/S3]; N2 τ² anchor = own T1/A2 three-component estimates ×{0.5, 1, 2} primary, COE-companion values demoted to a secondary "one-level lower-bound" scenario ×{0.5, 1, 2} with an explicit variance-structure mapping [S4]; N7 B = 2,000 → 500 (pre-registered benchmark rule fired at t_REML = 41.92 s) and the permutation object redefined as a **cluster-level time translation** with per-replicate re-derivation of all pp_* codings — H4 and H5 share one pre-generated, scheduling-invariant permutation list (seed 20260710) [S5/S8]; N10 = **113** cumulative steps (the two DEC-042b no-window studies are unplaceable; CI display from step 10, floor study "Chen, Gao (2011)") [S7]; N11 three-tier window rule (< 5 clusters not_estimable · 5–9 descriptive · ≥ 10 full inference; anchor 1998, 21 windows) [S6]; N12 scope shift to a descriptive channel-level comparison against the COE study-level row (no Paris moderator exists in the companion) [DEC-045]; E3 extended by the H2b moderation BF with the pre-registered "E2 + level BF" configuration as fallback [S1]. The TOST pins (E1/E2 incl. the ex-ante E1 expectation), the language-gate ladder, the H1 precompute stream with its 200-replicate REML calibration subset, the RoBMA data bases and priors (incl. the H2b contrast prior Normal(0; 0.025)), the three-tier v12 overlap disclosure (7/120 · 6/115 · 5/113; 60/2,713 ES ≈ 2.2 %; 0 shared effect sizes; supersedes Datenagenda #11), and all frozen constants live in DEC-045. F66 opened: rjags/BayesTools/RoBMA lockfile closure in the F64 pattern incl. the `RoBMA.reg` version pin and the JAGS-4.3.2 audit trail.


**A.14 — T7 / Block-C execution pins (2026-07-28; implements DEC-043 + DEC-048)**

- **Scripts / contract:** `R/10_moderators.R` + `R/10_verify_outputs.R` (paired, exit-0 required); executor contract `docs/cc_prompt_T7.md`; input contract `output/dat_prep.rds` (md5 `6702ef3dc45fe0b693b13f50ebd1576b`, n = 2,713, seed 20260710); pre-execution diff review mandatory (foundational script).
- **Panels (C1–C8):** M_A `zi ~ 0 + moderator` on estimation set ∩ classified; M_B `zi ~ 0 + cell(moderator × pp_mid_lag0)` (nonempty cells) on period ∩ classified; PI per level from the three fitted variance components (A.8 extension); pairwise contrasts among substantive levels (C2: 6, C5: 3, else 1); per-level Post−Pre diffs (z-only); interaction HTZ per panel; Holm line over the 8 interaction-HTZ p's.
- **NCE regime:** 99_NCE excluded from all C1–C9 analyses (C0 informational only). Per-panel domains (ES/studies/clusters) pinned: C1/C2 M_A 2,713/115/114, M_B 2,705/113/112 · C3 M_A 2,260/94/93, M_B 2,254/93/92 · C4 M_A 1,804/85/84, M_B 1,796/83/82 · C5 M_A 1,927/91/91, M_B 1,919/89/89 · C6 M_A 1,622/80/79, M_B 1,614/78/77 · C7 M_A 1,956/96/95, M_B 1,948/94/93 · C8 M_A 1,663/83/82, M_B 1,655/81/80. M-pre: C3 1,754/74/73 · C4 1,465/64/63. C9-V1 CC 1,762/80/80 (period 1,754/78/78; post 315/19/19) · C9-V2 CC 1,330/58/58 (period 1,324/57/57; post 117/9/9). Exclusion-disclosure row per panel; B5 boundary sentence in Methods.
- **regulation3:** `= regulation_sample_end` → {with ETS/CT, without ETS/CT, 99_NCE} [DEC-048 Q11a]; run-time gate post clusters 14/6/11 else STOP; one any-with sensitivity refit (C4-M_B, appendix, descriptive) [Q11b].
- **Small-cell double rule:** cell < 3 clusters → descriptive (no p); contrast df_Satt < 5 → no p, flag; HTZ always reported with own df + cell flag; not_estimable rows always keyed [P-T5-4 semantics; signature list {"converg","opposite sign"}; unlisted → S5; API mismatch → fix-zone]. Ex-ante pins: CDS×post = 0 · US×post = 1 · common×post = 2 · developed/rating/sensitive×post = 3 · without×post = 6; V2 country blocks structurally non-identifiable (single post level each).
- **Coding-absence disclosure rows (design facts, frozen):** C2/CDS end-post 299/10/10 all majority-pre · C5/US 551/16/16 with 550/15/15 majority-pre · C8/common 612/25/24 with 606/23/22 majority-pre.
- **C9:** V1 = pp + {CER_measure, COD_instrument, regulation3, country_region} + mechanically admitted interaction blocks; V2 = all 8 mains + pp + candidate blocks under the same rule; references performance · loan (interest rate) · without ETS/CT · 1_US (V2 additionally non-sensitive · 1_developed · 1_western · 1_common law); design-df rule ≥ 5 with identifiability precondition; echo rows (V1: 4, V2: 8).
- **Row budget = 200:** M_A levels 19 + pairs 15 + omnibus 8 · M_B cells 38 + diffs 19 + interaction HTZ 8 · Holm 1 · M-pre 10 · any-with 7 · disclosure 8 + 3 · V1 20 · V2 32 · echoes 12. Verifier recomputes the budget from design columns; four-key uniqueness; 36-column T2/T8 schema (header asserted against committed `output/T8_results.csv`).
- **Identity gates (dual):** pre-flight F65 (T1_results.csv A5 row by value, 1e-9, + pinned aggregate re-fit |Δ| ≤ 1e-6, df = 113) · verifier 3L headline re-fit value-matched to T1_results.csv (estimate + three variance components, 1e-6).
- **Outputs:** `output/T7_results.csv` (200 rows) · `output/T7_background_C0.csv` (background-only, absent from results — verifier absence check) · `output/T7_run_meta.txt` (versions, seed, column mapping, per-fit convergence certificates, timings, gate echoes) · `output/T7_sessionInfo.txt`. Language regime per DEC-047 throughout.
- **Runtime:** 22 REML fits (8 M_A + 8 M_B + 2 M-pre + 1 any-with + V1 + V2 + verifier headline re-fit) + cheap aggregate anchor; Kostprobe = first M_A fit timed, projection printed, run continues (cc_prompt milestone M1).
## Addendum A.15 — TG / Block-G execution pins (2026-07-29; implements DEC-050)

Run tag TG; scripts R/11_robustness.R + R/11_verify_outputs.R; cc contract docs/cc_prompt_TG.md. Engine: G1–G5, G7–G10 on the verbatim 3L-RVE spine (rho 0.6, CR2/Satterthwaite, optimizer ladder A.2/DEC-031f); G6 off-spine cluster-median rma.uni(REML, knha). Definitions: G1 drop es_method=="star-bound" (99/11 assert); G2 drop dat_prep R-19 proxy-fill flag rows (297 assert; xlsx columns non-canonical); G3 vi = 1/(n_firms − adj), adj = n_obs − 1/vi, integer-share ≥ .99 assert, invalid rows dropped+counted; G4 es_method=="bivariate" (near-identical to G9 bivariate cell, Δ = 2 ES/1 study, disclosed); G5 PCC vi = 1/(n − k − 3), k ∈ {10, 20}; G6 median-carrying-row vi convention; G7 q_VHB incl. 99_NCE (feature high vs low; D5 cross-ref); G8 field; G9 ES_measure (featured axis); G10 es_method main-effect-only. Inference: no spec-vs-headline tests (N5 multiverse is the single inferential home for spec variation); panels descriptive — cells + HTZ + pairwise CR2/Satterthwaite, no Holm, DEC-047 wording; no PIs (A.8 single home); Δ-vs-headline and flags formula-derived at register/workbook stage only. Gates: F65 dual gate by value (script aggregate + verifier 3L canary −0.062); domain 2,713/115/114 on dat_prep md5 6702ef3d…. Stops: S-A/S-G1/S-G2/S-G3/S5/S-SCHEMA/S-BUDGET; fixzone = FIXZONE block + accessors. Budget ≤ 60 rows / ≤ 14 fits / < 5 min; seed 20260710; 36-col header transplant from T7_results.csv. Outputs: TG_results.csv, TG_run_meta.json, TG_sessionInfo.txt; feed TG.md; workbook TB-65 ff.; robustness_register.csv assembled at workbook stage, status "partial — F rows pending" until the F session.

## Addendum A.16 — TF / Block-F execution pins (2026-07-29; implements DEC-051)

Run tag TF; scripts R/12_outliers.R + R/12_verify_outputs.R; cc contract docs/cc_prompt_TF.md. Engine: all refits on the verbatim 3L-RVE spine (rho 0.6, CR2/Satterthwaite on cluster_id, optimizer ladder A.2/DEC-031f); domain 2,713/115/114 on dat_prep md5 6702ef3d…; seed 20260710. Identification: F1 = studentized deleted residuals via metafor rstudent on the spine fit, ES-wise, reestimate = FALSE (convention disclosed), threshold |t| > 3 (conditional rule resolved — no threshold row in committed T1_results.csv; re-verified 2026-07-29); F6 = Leys MAD on zi, b = 1.4826, k = 3, full estimation set, computed once; both persisted per ES in TF_influence.csv (== 2,713 assert); N5/H3-mask relation disclosed descriptively, no test. Handling: outlier_rstudent / outlier_mad drop-and-refit (two rows, overlap disclosed, no union); winsor = zi at empirical P1/P99 of the full estimation set, once (identical to the A.13/H3 pin); unwinsor ≡ headline (DEC-013(a) status-quo semantics; cross-reference in the winsor row note, no fit); trim_1_99 symmetric on the same percentiles; leave_one_out = 114 cluster-deletion refits + CR2 (full table TF_loo.csv == 114 assert; W = 11 PSOCK admissible; register = one summary row, panel style); influence diagnostics descriptive — cluster Cook's D derived from the LOO fits (D_j = (μ̂ − μ̂₍₋ⱼ₎)²/Var̂(μ̂), full-set model-based Var̂), ES hatvalues from the full-set fit, no thresholds, register row "diagnostic — no refit". Inference regime unchanged: no spec-vs-headline tests; Δ-vs-headline and flags formula-derived at register/workbook stage only. Gates: Gate A = F65 aggregate by value 1e-9 (script + verifier); Gate B = verifier 3L refit value-matched against committed output/T1_results.csv, label-key analysis_id == "A1", 1e-6, no hand constants [DEC-048/050b/c], mirrored verbatim from R/11_verify_outputs.R. Stops: S-A/S5/S-SCHEMA/S-BUDGET/S-F1 (API toy-probe)/S-F4 (LOO ne > 5); structure asserts only. Budget: REML fits ≤ 130 (planned 119); TF_results.csv ≤ 40 rows (36-col header transplant from committed T7_results.csv; variant refits + ne rows only, no headline echo row). Outputs: TF_results.csv, TF_loo.csv, TF_influence.csv, TF_run_meta.json, TF_sessionInfo.txt; feed manuscript/TF.md (tracked feed home = manuscript\ per DEC-051); register rebuilt at workbook stage to 23 × 18, status "complete" (7 F rows incl. loso_post_cell cross-reference — values single-homed in T2/B6). Language DEC-047 throughout. **Corrigendum note (carry #5):** the A.15 gates line "verifier 3L canary −0.062" pre-dates DEC-050b; the 3L verifier leg is Gate B per DEC-048/DEC-050b/c (refit value-matched against committed T1_results.csv). A.15 text stands unedited (append-only); this note governs.

## Addendum A.17 — SD(COD) benchmark constants & construct-validity screen (2026-08-06; implements DEC-012b + DEC-062)

Constants landed (C1 extraction, 108 eligible studies, Cowork/Opus in four batches with corpus-evidence file matching): **A6 re-keys to `bp_per_1sd_{loan,bond,cds}`** with SD_COD_bp = **loan 200.0 (k = 49) · bond 150.0 (k = 21) · CDS 168.6 (k = 7)** — two-stage medians (within study × class, then across studies; equal study weight) over `data/benchmarks/p1_rows_merged.csv`; deterministic pipeline `R/13_bp_benchmarks.R` + paired `R/13_verify_outputs.R` (17/17), raw provenance `data/benchmarks/raw/` (14 files); hub numbers-QA PASS 2026-08-06. Coverage denominator per closed status list: 80/109 cells bp_ok (loan 52/72 · bond 21/26 · CDS 7/11); bp_ok counts mechanical convertibility before adjudication — the loan primary basis is k = 49 (− Azmi/Cicchini/Zhou 2018, arithmetically defective source tables). k-floor rule k ≥ 5 / 3–4 fragile / ≤ 2 → DEC-012 option (c) — not invoked. Row metadata: `scale_type`, `data_basis`, `unit_resolution`, `label_match`, plausibility corridor 10–1,000 bp. Sensitivity ladder (pre-declared): spread-only (loan 111.7, k = 10; bond/CDS ≡ primary), high-confidence-only (row filter confidence == high, then two-stage; 157.0/153.7/159.7, k = 37/12/6), unflagged-only (157.0/150.0/168.6), lognormal back-transform as **lower bound** (eligibility: log cells with printed μ and σ, `scale_type ≠ other`, bp-base assumption enforced by a 10–2,000 corridor; loan 12 → 7 cells, bond 1 → 0, CDS 3 → 3; SD_raw = exp(μ + σ²/2)·√(exp(σ²) − 1); loan 186.5 · CDS 163.0; single dual-scale validation case understates by ~28%); non-spread loan 310.0 (k = 39) as composition disclosure. Benchmark-scoped exclusions (rows, not studies): three arithmetically self-contradictory source tables (Zhou 2018 · Azmi 2021 · Cicchini 2026), one non-coded outcome row (Boermans YTM), one subsample selection (Kleimeier & Viehs Panel B per rule 8). **Release structure [DEC-012b]:** Commit A = pipeline + registers (this addendum; readiness Items 5A, 18, 19). Commit B = `R/01_core.R` A6 re-key + `R/01_verify_outputs.R` O15 flip + deterministic T1 re-run — **scheduled into the S1 window and executed jointly with the ERROR-#58 cluster merge** (author ruling 2026-08-06: merge 119→118 / 114→113, result-blind; own data-DEC + v12.x erratum), i.e. one single T1 re-run on the final structure; only `output/T1_results.csv` + `output/T1_run_meta.txt` carry, figures git-restored per DEC-061 §4; S1a/S1b split on hub signal if #16 delays. Sensitivity ordering: the descriptive fingerprint screen (Item 19; 80 covered cells, 29 uncovered disclosed) runs **before** any sensitivity; the combined drop-set row (Item 18: Du 2022 + Sandra/Ofogbe [DEC-062] + the defective-descriptives trio) runs on the merged structure. **Construct-validity screen [DEC-062]:** printed-mean screen over the 80 covered cells identifies three amount-scaled outcomes (Du et al. 2022; Sandra/Ofogbe cluster; 8/2,852 corpus ES, 2 clusters) — retained; Methods criterion 3 tightened to rate-or-price in the S1 micro-package (logged-exception regime); screen inapplicable to the 29 uncovered cells (disclosed).
