...
## DEC-041: v11 DATA FREEZE — scope cutoff (author decision; reviewer dissent documented), rulings formalized, native derivation grid populated corpus-wide; verifier 26/26
**Block:** Corpus governance / data freeze · 2026-07-10
**FREEZE:** data/CER-COD_data_v11.xlsx is the frozen raw+derived data basis: 2,852 effect rows · 2,851 estimation rows (1 duplicate-tagged) · usable ES set 2,730 · 122 documented non-convertibles (post gap-run, row-level quotes) · 120 studies = 60 legacy (65 v10 keys − 5 rule exits) + 60 new · clusters: Sandra/Ofogbe, Kleimeier-Viehs. Native v10 derivation & moderator columns populated corpus-wide from the coalesced window/country base (row → v10 study → log study-block → gap run): sample_mid/median, sample_post-share_2016–2019, pp_share/end_lag0–3, pp_mid/median/start_lag0, pp_window_class, distribution splits; industry (R4, incl. `mixed`), regulation_sample_start/end (E1–E4 resolver), country_econ via country_lookup + 24 confirmed extensions (100% coverage). Verifier 26/26 PASS (incl. V18–V20 parser/range, V21 positional top-up, V25 native-grid recompute, V26/27 coverage).
**Scope cutoff (the decision of record):** The July-2026 database sweep (Scopus 986 · WoS 1,033 · EBSCO 837, executed 2026-07-10 with corpus-vocabulary-derived strings; ScienceDirect omitted — its journal universe is Scopus-indexed; original six 2021 strings documented in the manuscript appendix) yielded 2,006 uniques → 1,720 screened (offline, per-abstract judgment): 647 include-candidates + 101 FLAG; after corpus rematch (−155) **593 non-corpus carry-forwards remain, concentrated 2024–2026 (70%)**. *Reviewer recommendation (Claude): Option A — precision pass → full-text → extraction tranches (+7–10 days), on completeness and Post-Paris-identification grounds; naked cutoff assessed as rejection-risk class.* **Author decision: Option B — extraction cutoff at 2026-07-10; corpus closes at 120 studies.** Mitigation package adopted as integral part of this DEC: (i) the cutoff is **result-blind** — set in this log before any pooled estimate exists (no outcome-dependence possible by construction); (ii) the sweep is documented as what it was commissioned as: a **recall-validation layer** for the targeted update (43/60 retrieved; misses fully attributable to non-indexed WPs, non-indexed journals, 2026 early access; backward test on pre-2020 hits run); (iii) the 593 candidates are **catalogued** (docs/dbrun_* + screening files) and will be reported as an identified-not-extracted set with year/theme structure; (iv) manuscript carries a quantified limitation + future-research framing (the 2024–26 wave as a distinct evidence generation); (v) **revision contingency:** the extraction pipeline (v1.6 workflow, verifier, adjudication-light) remains operational — executing Option A on referee demand is estimated at ~1 week.
**Formalizations:** Ruling session 2026-07-09 (S1–S19; docs/ruling_session_2026-07-09.md) and blankets B1–B4 (estimation_method FLAG → 'other: not stated' corpus-wide; CER maps D'Arcangelo/Chava → performance; Delis price-valued tags = 2 label-evidenced; the 13 structural-class value diffs stay staging-side). Gap-run rulings E1–E8 (Europe-only multis → with/with; global multis → category 'multi'; non-ETS countries → without; China national rule 2021; industry default 'mixed' when unreported; JIF vintage exception; windowless cross-sections per E7; ES rest = documented k-losses). Quality moderator = VHB only, JIF dormant (DEC-023 closed). Assembly tooling language boundary codified: Python for data engineering (container-executed, pinned inputs, committed); R is the analysis language from prep onward.
**Ledger correction (honesty note):** v10 contains 65 unique study keys, not 66 as carried in earlier balances; the prior "Höck unicode artifact" narrative is retired — the file was right, the ledger arithmetic was not.
**Reviewer-Risk:** *Finance/Econometrics* — coverage completeness: answered by result-blind cutoff, full candidate catalogue, k=120/2,730 (above field norms), and the 1-week revision contingency. *Management/BSE* — evidence currency: answered by the documented 2024–26 wave as future-research agenda and the recall-validated update layer.
...
## DEC-031: Final analysis battery locked — a-priori input fixation on v12 (resolves the DEC-029 battery-prep step and the DEC-030 reservation; embeds author rulings F48–F52, P-01–P-10, r02f2, r15; unified-model composition split out as Pending-D)

**Block:** Methodology finalization (Step 2, post-v12) · 2026-07-10

**Question:** With `data/CER-COD_data_v12.xlsx` canonical (DEC-042: 2,852 rows · 2,730 usable ES · 120 studies · 2 clusters), which analyses constitute the final, result-blind battery, and which numerical inputs are fixed before any model run?

**Options considered:** (a) run analysis_plan.md as written on stale quantities; (b) consolidated battery lock with mechanical re-derivation of all design quantities on v12 and every open operationalization ruled; (c) defer contested pieces to first-run inspection (excluded per DEC-005).

**Chosen:** (b). Embedded decisions, all author-ruled 2026-07-10:

- **D31.1 — Inference unit = `cluster_id`** [CONFIRMED]: all random-effects nesting (`~1|cluster_id/study/esid`, collapsing where clusters are singletons) and all CR2 clustering; supersedes `cluster = study` snippets in analysis_plan §§3–8. Sensitivity `cluster_study` reported once.
- **D31.2 — Effect-size set** [CONFIRMED]: ALL usable effect sizes enter the headline, star-bound rows included with the conservative bound (99 rows / 11 studies; count in Methods); single sensitivity `no_starbound`. Star-only runs rejected (answer no posed question).
- **D31.3 — DEC-040 honesty notes: disclosure-only** [AUTHOR RULING]: provenance exclusion runs (`staging_only`, `adoptions_only_drop95`, `drop_zhou_cubas`) are NOT executed. Reviewer dissent documented (a disclosed asymmetry invites the R1 request; pre-computing is cheaper); author position: adjudicated rows are legitimate data, a sensitivity would imply unwarranted doubt. Contingency: specs remain executable on demand; response-letter skeleton carries the prepared answer.
- **D31.4 — q_VHB operationalization** [P-01/P-08/P-09 final]: WP → `99_NCE`; published A+/A/B/C → `1_VHB high`; published D or not VHB-listed → `0_VHB low`. Cells on v12: 76 / 30 / 14 studies (1,635 / 524 / 693 rows). Raw letters canonical in lookup; rule lives as a visible formula.
- **D31.5 — Moderator architecture** [P-06/P-10 resolution]: Stage 1 = **univariate moderator panels for the full inventory** (CER_measure, COD_instrument, industry, regulation3, country_region, country_econ, country_culture, country_legal, q_VHB, q_status grey-literature panel, field, ES_measure, es_method, corpus_segment) — each `zi ~ pp_share_lag0 * moderator` plus main-effects-only variant, CR2 on cluster_id. Stage 2 = **unified model(s): composition NOT fixed here**; it is decided in a dedicated session strictly BEFORE the first unified-model run (result-blind compatible: decision precedes results; possibly multiple pre-registered variants V1/V2/…, ALL reported, no post-hoc selection) → **Pending-D**, resolves as DEC-043. The reviewer's specification-search concern and the author's wish to see univariate structure first are both satisfied by this ordering: univariate panels are themselves pre-specified here and their results do not license dropping any pre-registered unified variant.
- **D31.6 — Sample-size basis** [R-16b/R-19/r02f2]: variance/weighting df basis = column E (n_obs where numeric, else n_firms × window-years proxy; 297 proxy rows incl. 134 FLAG-based provisional per R-19); 16 rows without any n drop from estimation (effective estimation set 2,714 of 2,730; both figures reported). Sensitivities: `no_nobs_proxyfill` (drop all 297 proxy rows) and **`n_firms_variance`** (author r02f2: re-run variance/weighting with n_firms as basis).
- **D31.7 — Background diagnostic** [author r15]: correlation/association matrix (Cramér's V / polychoric as appropriate) of the country moderators (region, econ, culture, legal) incl. regulation overlap, run BEFORE moderator analyses; background-only, not for manuscript; informs the Pending-D session.

**A-priori quantities:** every legacy cell count in the annex table below is a placeholder; T0.4-prep re-derives ALL design quantities mechanically on v12 into `output/design_quantities_v12.csv` (the citable numbers). Roles and conditional rules — not counts — are what is fixed: headline assignments, and the DEC-024 inference-transfer rule (design-only Satterthwaite df of the Paris coefficient: df ≥ 5 → full inference on the binary; df < 4–5 → binary descriptive, inference transfers to the continuous dose model).

**Battery (annex, roles fixed):**
- **A. Core (T1):** 3LMA-RVE pooled mean (Fisher-z PCC per van Aert 2023, ρ = 0.6, CR2/cluster_id) · variance decomposition · prediction interval · ρ ∈ {0.4, 0.8} · estimator robustness (one-effect-per-cluster; UWLS+3; HS+WAAP appendix) · bp translation (DEC-012).
- **B. Paris identification (T2/T8), DEC-024 architecture unchanged:** headline binary `pp_share_lag0 ≥ 0.5` (ties→Post) subject to the transfer rule · continuous `post_share` dose model as first-class identification (z̄(0)/z̄(0.6)/z̄(1), in-support contrast, quadratic check) · tie_break (`pp_mid`) · `end_any_exposure` upper-recall bound · share-lags 1–3 main-text coding sensitivity (collapse = finding) · end-lags appendix · clean-window descriptive · trend-vs-break race, placebo break years, segmented MR, composition control (Move 4 core), bounding · LOSO on the post cell · attenuation/knife-edge/panel-drift disclosures re-computed on v12.
- **C. Moderators (T7):** per D31.5; `es_method_factor` always main-effect-only (method artefact, never interacted); cells < 3 clusters → descriptive-only per the df rule.
- **D. Publication bias (T5):** FAT-PET-PEESE (CR2, cluster_id; a-priori rule: PEESE only if PET intercept significant, same rule in every subgroup) · 3PSM (`selmodel`, step 0.025) + p-uniform* on one-effect-per-cluster aggregates · pre/post split (N14, appendix) · RoBMA row (N3) · triangulation statement feeds Pending-C.
- **E. Equivalence (N1):** TOST, SESOI |r| = 0.070 primary (F27v2), |Δr| = 0.05 secondary — both reported, primary adjudicates; applied to pooled mean and the Paris contrast (dose contrast 0→0.6 if transferred). N1+N3 jointly fix the "evidence of absence" wording (Pending-C input).
- **F. Outliers (T4), Aguinis/Joo · Leys · DEC-013:** rstudent identification → drop-and-refit (prominent) · winsor/unwinsor pair (never primary) · cluster-level LOO + post-cell LOSO · influence diagnostics appendix · all treatments pre-stated, no result-contingent trimming.
- **G. Robustness catalogue (single long CSV, `spec` column):** carried set (leave_one_out, loso_post_cell, outlier_rstudent, winsor/unwinsor, one_effect_per_cluster, cer_type, q_vhb_published, status_as_extracted, event_coding suite, uwls3, direct_r_only, pcc_df_k10/k20, rho_0.4/0.8, cluster_study) **plus v12 specs:** `no_starbound` · `no_nobs_proxyfill` · `n_firms_variance` · `segment_original_only`. *(Provenance specs removed per D31.3; retained there for on-demand execution.)*
- **H. Null battery N1–N15, inputs FIXED [P-07 approved]:** N2 MDE simulation: τ²/ω² = COE-companion estimates × {0.5, 1, 2}, B = 5,000, seed 20260710 · N3 RoBMA: PSMA defaults + Zarea-informed Normal(μ, σ) prior — **[single external PENDING: extraction from Zarea et al. 2026]** · N5 multiverse: {6 Paris codings} × {ρ .4/.6/.8} × {outlier none/rstudent/winsor} × {ES set full/no_starbound} × {estimator 3LMA-RVE/UWLS+3/one-per-cluster} × {df E-basis/k10/k20}, spec-curve permutation B = 500 · N6 sup-break grid sample_mid ∈ {2008…2019} · N7 cluster-permutation B = 2,000, seed 20260710 · N8 Zarea transplantation · N10 cumulative MA (main time graphic) · N11 rolling window width 6 y / step 1 y, min 5 clusters · N12 external-difference test vs COE companion · N13 p-curve conditional (≥ 5 significant study-level post-cell p; first-reported selection; else "infeasible") · N15 within-study display.
- **I. Execution & gates:** T0.4-prep (+verifier incl. formula≡script identity, closed lists, grid identities, audit re-run, cross-language recomputation of ALL derived columns from raw fields) → T1 dry-run → pre-execution diff-review gates for T0.4/T1/T7/T8; verifier-paired elsewhere; per-run results workbook with Manuscript-Inputs tab; result-blind holds until this DEC is logged.

**Rationale:** Closes every operationalization opened by v11→v12 with an explicit author ruling on the record; fixes roles, rules, seeds and grids before any estimate exists; splits the one genuinely unresolved design choice (unified-model composition) into a pre-run Pending rather than an after-results decision.

**Reviewer-Risk:** *Finance/Econometrics* — the battery reads as a pre-registered program; D31.5's two-stage structure with Pending-D documents that unified-model composition was fixed before estimation. *Management/BSE* — D31.3 dissent is on the record with a prepared R1 answer; the univariate panel battery plus composition diagnostics (D31.7) pre-empt the multicollinearity objection.

**Consequences:** analysis_plan §§3–9/13 to be updated to v12 quantities and cluster_id (ships with the T0.4-prep commit); Status `Null_Battery` inputs column fillable; **Pending-D opened** (unified-model composition session; resolves as DEC-043 before R/05-T7 unified runs); Pending-C inputs unchanged (N1/N3 + T8 verdict); single external pending = Zarea prior extraction (N3).
...
## DEC-031g: T5/Block-D execution spec — 3PSM base RE/ML (F63), puniform lockfile closed (F64), A5 anchor protocol (F65), D5 cell inventory pinned, D3 → H-session

**Block:** Methodology finalization (execution amendment to DEC-031 Block D) · 2026-07-18

**Question:** Which operational pins govern the T5 run of Block D (D1 FAT-PET-PEESE, D2 selection models, D4 pre/post split, D5 grey-literature panel, D6 triangulation), given (i) the plan-§8.2 legacy snippet (FE base, "one-effect-per-study, k = 66") conflicts with the pinned "3PSM" label and the cluster-level architecture, (ii) puniform was absent from renv.lock, (iii) DEC-031f binds all spine fits, and (iv) the battery D3 row shares its machinery with H2/N3?

**Options considered:** F63: (a) RE/ML base for selmodel — 3PSM proper {μ, τ², δ} — vs (b) FE base per the §8.2 snippet (2-parameter Hedges–Vevea). F64: (a) renv install + explicit record, lockfile ships with the T5 package commit vs (b) drop p-uniform* (conflicts with DEC-016/DEC-031). D3 scheduling: (a) execute once in the H-session, cross-reference from D3 vs (b) duplicate the RoBMA run in T5. F65 anchor source: (a) verifier reads the committed output/T1_results.csv at run time, re-fit independently pinned vs (b) hand-frozen constants pasted from that CSV.

**Chosen (author rulings 2026-07-18):**
- **Scope:** T5 = D1, D2, D4, D5 + D6 as DRAFT (RoBMA line BLOCKED until H2). D3 executes in the H-session [DEC-031b]; the battery D3 row gains the "→ H2" cross-reference at the T5 status touch.
- **D1:** spine per D31.1/A.2/A.11 (V ρ = 0.6; ~1|cluster_id/study/esid; REML sparse; CR2/Satterthwaite on cluster_id) with mods = ~sez (intercept = PET, slope = FAT) and mods = ~I(sez^2) (PEESE); domain 2,713 ES / 115 studies / 114 clusters. Continuous regressors ⇒ DEC-031f ladder mandatory, optimizer per fit in run_meta, one BFGS control re-fit (R6, |Δβ| < 1e-5).
- **P-T5-1:** PET and PEESE are always both computed and reported (DEC-010 "side by side"); the conditional rule (PET intercept, two-sided α = .05, CR2/Satterthwaite) only sets a mechanical `rule_selected` flag — identical rule in every subgroup.
- **P-T5-2:** sez ≡ sqrt(vi), derived in-script; assert all(is.finite(vi) & vi > 0); the raw v12 SE column is never read by T5; PEESE regressor I(sez²) ≡ vi; provenance recorded in run_meta.
- **F63 = (a):** D2/D4 selection models on one-effect-per-cluster aggregates (metafor aggregate.escalc, cluster = cluster_id, struct = "CS", ρ = 0.6 — the T1/A5 convention): base rma(yi, vi, method = "ML") → selmodel(type = "stepfun", steps = 0.025, alternative = "less"); p-uniform* via puni_star(side = "left", method = "ML"). Direction pins jointly asserted (selection favors significant negative effects).
- **P-T5-4 (sharpened):** selmodel/puni_star are off-spine — the DEC-031f ladder does not apply. A pinned non-convergence signature list — initialized to {"converg", "opposite sign"}, case-insensitive substrings; author-reviewed with this package; extendable only by DEC — classifies documented non-convergence ⇒ `not_estimable` rows with the full condition text in the note field (all planned keys are still written; failures never surface as missing rows); any non-listed condition ⇒ S5 stop, non-zero exit. Package-API field mismatches raise a labelled fix-zone stop (accessor-only repair), distinct from S5.
- **D4:** period domain 2,705 (pre 1,994 ES / 83 studies · post 711 / 31 / 31 [DEC-042b]); FAT-PET-PEESE per period under the identical rule; conditional upgrade FIRED [DEC-031a.5, post cell 31 ≥ 20]: 3PSM + p-uniform* additionally on per-period one-per-cluster aggregates.
- **D5:** main-effects-only cell-means (zi ~ 0 + q_status) on the spine, CR2, between-group contrast + HTZ Wald; the Paris-interaction variant goes to the on-demand register. Cell inventory pinned ex ante (F60 pattern), derived result-blind from v12 design columns with certified filter replication (margins 2,713/115/114 and pp_mid cells 1,994/83 · 711/31 · NA 8/2 reproduced exactly): `0_published` = 2,033 ES / 101 studies / 100 clusters; `1_not published` = 680 / 14 / 14. Both cells ≥ 3 clusters ⇒ full inference; the single multi-study cluster lies inside the published cell. Corpus cross-check, asserted on v12 RAW rows (verifier O-check, labels verbatim): q_VHB studies `1_VHB high`/`0_VHB low`/`99_NCE` = 76/30/14 and q_status = 106/14 at corpus level (2,852 rows / 120 studies); on the usable set (2,730 / 117) these read 74/29/14 and 103/14 — the corpus figures include the three zero-usable studies (two VHB-high, one VHB-low; all published); estimation-set q_status = the pinned 101/14.
- **F64 closed (author, 2026-07-18):** findstr on renv.lock empty (incl. /I) → install puniform@0.2.8 (linked from cache) → renv::snapshot no-op (implicit snapshot mode; no code reference before script placement) → explicit renv::record("puniform@0.2.8") → lockfile entry l. 2886 ff., packageVersion 0.2.8; git: modified renv.lock. Disclosed: one inadvertent double record (idempotent) and the expected transitory "out-of-sync" state until the confirmation snapshot. Conventions: renv.lock ships in the T5 package commit; after script placement, one renv::snapshot(prompt = FALSE) as confirmation, any additional delta in the same commit.
- **F65 = (a) + embedded secondary canary (author, 2026-07-18):** the verifier loads the committed output/T1_results.csv at run time and locates the A5 one-per-cluster row BY VALUE against programmatically extracted full-precision constants (est −0.0616608386540629 · se 0.0243306602731784 · df 113 · p 0.0126367644358383 · CI [−0.109864264918875; −0.0134574123892513]); uniqueness asserted; all fields hard-checked at 1e-9 against the runtime-read row (CSV-drift canary), df exact, display-rounding canary −0.062. It then re-fits via the independently pinned call — metafor aggregate.escalc (cluster = cluster_id, struct = "CS", ρ = 0.6) → rma.uni(method = "REML", test = "knha") — and asserts |Δ| ≤ 1e-6 on estimate/SE/p with df = 113 (= k − 1, KnHa on 114 aggregates), plus the same identity on the T5 CSV anchor row. Disclosed deviation from the hand-frozen-constant letter of the O15 pattern: the runtime source is the canonical committed CSV; the embedded constants restore the freeze as a drift check; the identity logic additionally validates the pinned call (a mismatch S-stops rather than silently reproducing copied code).
- **P-T5-5:** no bp rows in T5; economic translation remains A6/DEC-012a territory (constants pending, Volker).
- **Status touch (ships with the post-run commit):** DecisionLog_Index gains DEC-031f (deferred; documented no-op in the T8 package-2 edit, project error log #7) and this entry — count 51 → 53; Key_Results Z11 filled with the author-approved framing-neutral string: "Trend −0,0024/J (p=0,113) · Bruch 2016 +0,026 [−0,026; +0,079] (p=0,306) · Placebo-Rang 2016: 7/9 (|b| 2009–11: 0,033–0,040) · Segment-Joint p=0,525 · Dosis: Slope +0,052 (p=0,271), Kontrast 0→0,6 +0,031 [−0,026; +0,088], Joint p=0,207 · Komposition: Δβ +0,002, adj. pp +0,012 [−0,025; +0,050] · Bounding: β*(δ=1)=+0,012, δ*=9,28, E-Value 1,19/CI-Rand 1,43, Masks 1,42/1,53 — Verdict-Text: Pending-C" (source: output/T8_results.csv; output/T8_results_workbook.xlsx @ 62d80d4); Z12 gains the T5 line; Block-D battery rows completed with hashes; D3 cross-reference set.
- **Process note (project error log #8, Claude, 2026-07-18/T5):** the initial T5 gating list was incomplete — the T8 pattern references (36-column schema, dat_prep input contract, run_meta/certificate format) were not requested until the package-build turn; closed by author upload of both R/08 scripts (verifier md5 1212deb5… confirmed in-container).

**Rationale:** The "3PSM" label in DEC-016 and DEC-031 governs over a legacy FE snippet — with 62% cluster-level heterogeneity share (T1/A2), τ² belongs inside the selection model, and Carter et al. (2019) / van Aert & van Assen operate ML-based. Direction pins prevent a silent wrong-side selection model. Ex-ante cell pins (F60 pattern) turn D5 into design facts before any estimate exists. Single-machinery D3 avoids a duplicated Bayesian run and keeps T5 frequentist; the Pending-C trigger needs PET only.

**Reviewer-Risk:** *Finance/Econometrics* — "did you shop optimizers/estimators?" is answered by the DEC-031f ladder disclosure, the a-priori PET→PEESE alignment rule, and the pre-registered direction of selection (left); ML-based 3PSM matches the cited methods literature. *Management/BSE* — the grey-literature panel sits in the bias block where a selection-minded referee looks; the triangulation statement is staged framing-neutral, so no A/C-framing leaks before Pending-C.

**Consequences:** analysis_plan Addendum A.12 ships in the same commit (supersedes the §8.2 snippet and stale §8 quantities); R/05_bias.R + R/05_verify_outputs.R implement this entry (deterministic row budget 53; verifier O1–O27); docs/cc_prompt_T5.md pins the canonical run (fix zone: syntax / package-API accessors / renv::restore only; no CC commits; no result framing); renv.lock joins the package commit (git-add list and --stat expectation updated; confirmation renv::snapshot(prompt = FALSE) after script placement, any delta in the same commit); F63/F64/F65 closed — F register empty, next number F66.
